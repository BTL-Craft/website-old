...
if (array_key_exists('url', $_POST)) {
    if (substr($_POST['url'], 0, 4) == 'user') {
        $url = substr(strchr($_POST['url'], '/'), 1);
        if ($_POST['url'] == 'user') {
            $view->load_twig_file(__DIR__ . '/../assets/view/', 'user.twig', $custom_texts);
            exit;
        } else {
            if (file_exists(__DIR__ . '/../assets/view/user/' . $url . '.twig')) {
                $view->load_twig_file(__DIR__ . '/../assets/view/user/', $url . '.twig', $custom_texts);
            } else {
        ...
        echo substr(strchr($_POST['url'], '/'), 1);
...
switch ($_GET['url']) {
    case 'auth':
        if (is_array($sesslogin)) {
            if (array_key_exists('state', $sesslogin)) {
